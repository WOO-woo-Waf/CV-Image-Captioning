...
class MLPConnector(nn.Module):
    def __init__(self, input_dim=1024, hidden_dim=4096, output_dim=4096, num_layers=10, num_queries=32):
        super().__init__()
        self.query_proj = nn.Linear(input_dim, input_dim)  # 用于构建 learnable query path

        # 初始化 Learnable Query
        self.query_embed = nn.Parameter(torch.randn(1, num_queries, input_dim))
    ...
    def forward(self, visual_feats):  # visual_feats: [B, N, 1024]
        B = visual_feats.size(0)
        query = self.query_embed.expand(B, -1, -1)  # [B, num_queries, 1024]

        # 将视觉特征压缩为 query 的输入（可选增强）
        query = self.query_proj(query)

        for block in self.mlp_blocks:
            query = query + block(query)  # 残差结构

        return self.out_proj(query)  # [B, num_queries, output_dim]
...
class VisionLanguageModel(nn.Module):
    def __init__(self, lm_path='/home/wangdx_lab/cse12210928/Qwen/qwen-7b-chat/qwen/Qwen-7B-Chat', freeze=True):
        ...
        self.connector = MLPConnector(input_dim=1024, hidden_dim=5120, output_dim=self.embed_dim, num_layers=10)
    ...
    def forward(self, images, prompt_ids, prompt_mask, cap_ids, cap_mask):
        ...
        prefix_emb = self.connector(vis_feat)
...
def train_single_gpu(accumulation_steps=32):
    # 读取数据
    ...
    torch.save(model.state_dict(), 'clip_mlp_full.pth')
    torch.save({
        'connector': model.connector.state_dict()
    }, 'lightweight_mlp_ckpt.pth')
...
def generate_caption_from_image(model_path, lm_path, image_path, device='cuda'):
    ...
    seq_ids = logits.argmax(dim=-1)[0]

    # prefix_len 是你在 Q-Former 中设置的 query 数量（比如 32）
    prefix_len = model.qformer.num_queries  
    # prompt_len 是 pad_prompt 的长度
    prompt_len = pad_prompt.size(1)

    # 从 prefix_len + prompt_len 后开始才是模型真正生成的 caption
    cap_ids = seq_ids[prefix_len + prompt_len :].cpu().tolist()
    generated_text = tokenizer.decode(cap_ids, skip_special_tokens=True)
...
def train_ddp(ckpt_path='clip_qformer_2.pth', accumulation_steps=32, num_epochs=2):
    ...
    dataloader = DataLoader(dataset, batch_size=1, sampler=sampler, num_workers=4, pin_memory=True)

    # === Model ===
    lm_path = '/home/wangdx_lab/cse12210928/Qwen/qwen-7b-chat/qwen/Qwen-7B-Chat'
    model = VisionLanguageModel(lm_path, freeze=True).cuda(local_rank)

    if os.path.exists(ckpt_path):
        map_location = {'cuda:0': f'cuda:{local_rank}'}
        if rank == 0:
            print(f"[Rank {rank}] Loading visual modules from {ckpt_path}")
    ...
    for epoch in range(num_epochs):
        ...
        if rank == 0:
            dataloader_iter = tqdm(dataloader, desc=f"Epoch {epoch+1}", dynamic_ncols=True)
        else:
            dataloader_iter = dataloader

        for step, (images, captions) in enumerate(dataloader_iter):
            images = images.cuda(local_rank, non_blocking=True)
            prompts = ["Describe this image"] * images.size(0)
            p = tokenizer(prompts, return_tensors='pt', padding='max_length', truncation=True, max_length=256)
            c = tokenizer(captions, return_tensors='pt', padding='max_length', truncation=True, max_length=256)
            for k in p: p[k] = p[k].cuda(local_rank, non_blocking=True)
            for k in c: c[k] = c[k].cuda(local_rank, non_blocking=True)
            ...
            if (step + 1) % 100 == 0 and rank == 0:
                generated_ids = outputs.logits.argmax(dim=-1)
                generated_text = tokenizer.decode(generated_ids[0], skip_special_tokens=True)
                print(f"[Rank {rank}] Step {step+1} - Gen: {generated_text}", flush=True)
                writer.add_scalar("Loss/train", loss.item() * accumulation_steps, epoch * len(dataloader) + step)
                lr = optimizer.param_groups[0]['lr']
                writer.add_scalar("LR", lr, epoch * len(dataloader) + step)

        if rank == 0:
            avg_loss = total_loss / len(dataloader)
            print(f"[Rank {rank}] Epoch {epoch+1} done, avg loss: {avg_loss:.4f}")
            writer.close()
            torch.save(model.module.state_dict(), f"clip_qformer_3.pth")
    ...
    with torch.no_grad():
        for step, (images, captions) in tqdm(enumerate(dataloader), total=len(dataloader), desc="Evaluation Steps", leave=False, dynamic_ncols=True):
            ...
            with autocast(device_type='cuda'):
                ...
                # generated_ids = outputs.logits.argmax(dim=-1)
                # generated_text = tokenizer.decode(generated_ids[0], skip_special_tokens=True)
                seq_ids = outputs.logits.argmax(dim=-1)[0]  # [seq_len]

                # 2) 计算 prefix 和 prompt 的长度
                prefix_len = model.qformer.num_queries               # Q-Former query 数量
                prompt_len = prompt_tokens['input_ids'].size(1)      # e.g. 256

                # 3) 只取真正生成 caption 的部分，转为 CPU list，然后 decode
                cap_ids = seq_ids[prefix_len + prompt_len :].cpu().tolist()
                generated_text = tokenizer.decode(cap_ids, skip_special_tokens=True)
